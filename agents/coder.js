 ...
 */
export async function code(plan, projectContext, critique = '') {
  ...
  const label = critique ? '🔄 Coder iterating (critique applied)...' : '💻 Coder implementing...'
  console.log(label)
  ...
  if (Object.keys(changes).length === 0) {
    console.log('  ⚠️  Coder returned no parseable file blocks. Raw output saved.')
    return { __raw__: result }
  }

  console.log(`  ✓ ${Object.keys(changes).length} file(s) to write: ${Object.keys(changes).join(', ')}`)
  return changes
}
